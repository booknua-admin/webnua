...
//
// Three optional inputs inside one chat bubble:
...
export function ChatBrandPicker({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Logo */}
      <div>
        <label className="mb-1 block font-mono text-[10px] font-bold uppercase tracking-[0.14em] text-ink">
          Logo (optional)
        </label>
        <div className="flex flex-col items-stretch gap-2 sm:flex-row sm:items-center">
          ...
          {logoUrl ? (
            ...
            <img
              src={logoUrl}
              alt="Logo preview"
              className="h-12 w-auto rounded border border-rule bg-paper object-contain px-2 py-1"
            />
          ) : null}
        </div>
        {uploading ? (
    ...
    <div>
      ...
      <div className="flex items-stretch gap-2">
        <input
          ...
          className={cn(
            'h-11 w-12 cursor-pointer rounded-md border border-rule bg-card p-1',
