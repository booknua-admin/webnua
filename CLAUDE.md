...
> **Architectural note — funnels are client-only.** The prototype's funnel detail (client Screen 23) has no admin equivalent — operators interact with funnels through the page-builder flow (admin Screens 41/42/43), which is a different surface entirely. So everything funnel-shaped lives in `client/funnels/`, and the route nests under `/website/funnels/[id]` to match the prototype's "← back to website" nav hierarchy. `/website` is a `PagePlaceholder` for now — the full Website Overview (page-card grid, perf hero, request-change flow) is a future session.
...
- **`FunnelStepThumbnail`** (`funnels/FunnelStepThumbnail.tsx`) — small 80px-tall CSS "preview gist" of each step. `variant: 'landing' | 'schedule' | 'thanks'`. Pure visual, no business logic. Will also be consumed by the Website Overview's funnel card when that screen is built — kept in `client/funnels/` because both consumers are client-only.
...
- **`InkHero` — 4 ink-hero-style components exist** (`IntegrationProgressHero`, `BillingPlanCard`, `TicketsHero`, `FunnelHero`). Before extracting a shared primitive, **confirm they share structure, not just styling**. Right now they share a visual treatment (ink-bg + rust mono pill + ~30px headline + paper/70 subtitle) but each has a different shape underneath — `IntegrationProgressHero` has a fraction + gradient progress bar, `BillingPlanCard` has a plan-meta row, `TicketsHero` has a polymorphic `right` slot for actions/stats, and `FunnelHero` (Phase 5 funnels) embeds a paired `FunnelAggCard` on the right + a back-link + a version pill + a 🔒 tag prefix. **None of the four are the same component with different data — each has a meaningfully different right-column shape and different left-column extras.** Extracting now would still produce a grab-bag of optional slots serving four different layouts, which is worse than four honest components. **Hold pattern: if a fifth ink-hero appears and at least two of them visibly become structurally identical, that's the extract signal. Until then, consider a shared style recipe (a `cn(...)` helper or a documented Tailwind class string for the ink-bg + radial-flourish + rust-mono-tag chrome) rather than a component — the only thing all four really share is chrome.**
