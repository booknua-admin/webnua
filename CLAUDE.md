...
- **`IntegrationProgressHero`** (`settings/IntegrationProgressHero.tsx`) — the ink-bg rust-tagged progress band at the top of the client `/settings/integrations` page. Props: `tag` (rust mono pill string), `title` (ReactNode, `<em>` = rust highlight, 26px headline), `subtitle` (ReactNode, `<strong>` = paper-bold), `connected` + `total` (numeric — drives the big mono fraction + the rust→rust-light gradient fill), `remainingLabel` (ReactNode for the right-of-bar caption). One variant only — the admin cross-client matrix (Session B) needs a different shape (stat tiles, not a fraction), so build that one when it lands rather than overloading this.
- **`SettingsTabPlaceholder`** (`settings/SettingsTabPlaceholder.tsx`) — Card-shaped placeholder for the settings tabs that ship as stubs in Session A. Rust eyebrow + "Coming next session" heading + short note + dashed hint card. **Delete every usage when Session B fills in the remaining tabs** (Notifications/Billing/Security/Help on client; Team/Billing/Defaults/API/Danger on admin).
...
- **`SettingsPanel` — second-use trigger.** Phase 5 session A shipped the settings panel as a `Card + CardContent` composition inlined in each tab page (white surface holding the section blocks) rather than extracting a `SettingsPanel` component. When the **second** route reaches for the same `<Card className="py-7"><CardContent className="px-8">` pattern (likely happens immediately as Session B's other admin/client tabs land), extract it to `components/shared/settings/SettingsPanel.tsx` and replace both call sites. The shape is `Card` with vertical rhythm tuned for `SettingsSection` children — no other knobs.
