...
- **`LeadRailCard` vs `TicketSideCard` — second-use of the white-card-rail pattern.** Both are mono-uppercase-heading + white-card-surface + body. Built `LeadRailCard` separately because the row-value vocabulary differs (leads carries `tone: 'good' | 'quiet' | 'default'` + `accent: true` for rust tinting; tickets has plain bold value). **Hold pattern: if a third surface needs this rail card, extract a `RailCard` primitive that accepts the row-value tone bag as a slot.**
