...
- **`ContinueSetupHero`** (`ContinueSetupHero.tsx`) — "mid-setup" hero card built for an operator dashboard. White surface with rule border, `rounded-3xl`, `px-10 py-9`, two-column grid (content + right-aligned CTA). Props: `tag` (rust mono eyebrow), `title` (ReactNode, supports `<em>` for rust), `description`, `meta` (`ReactNode[]` — segments are separated by `·` and any `<strong>` renders ink-bold), `ctaLabel`, `ctaHref` (rendered through `Button asChild` + `next/link`). **Currently not mounted** — the `/clients` page was repurposed to the all-clients list and no longer renders this. Re-mount when the operator dashboard returns as its own route; don't rebuild.
- **`ClientListRow`** (`ClientListRow.tsx`) — all-clients screen row. White card surface, `rounded-lg`, 6-column grid (`32px 1fr 120px 100px 120px 80px`). Composes a 32×32 ink-bg avatar tile with a rust-light glyph, name + meta lines, a status-dot pill (`good`=live, `rust`=setup), `leadsPerWeek` (rust number), `spend`, and a rust "Open →" affordance. Whole row is a `next/link`. Props: `{ id, initial, name, meta, status: 'live'\|'setup', leadsPerWeek, spend, href }`. Re-export `ClientStatus` type. For an in-setup client pass `leadsPerWeek: 0` and `spend: '—'`.
...
### `lib/dashboard/` — per-role page stub data

- **`admin-dashboard.tsx`** — stub data for the admin `/clients` (all-clients) screen: `clientsPageHeader` (eyebrow + `title`/`subtitle` ReactNodes carrying the `<em>` rust highlight) and `allClients` (4 rows — Voltline in setup + 3 live). Exports the `ClientRecord` type. `.tsx` because header values are JSX. Re-shape into Supabase reads when the backend lands. Filename is now historical — the screen is "all clients", not a dashboard; rename when something else lands in this folder.
