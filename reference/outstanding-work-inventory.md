...
- **Effort.** Small.
- **✅ Resolved.** Migration `0046_realtime_website_approvals.sql` adds
  `website_approval_submissions` to the `supabase_realtime` publication;
  `RealtimeProvider` gains a `postgres_changes` subscription that
  invalidates the three approval query-key prefixes
  (`['website','pending-all']` / `['website','publish-state']` /
  `['website','pending-for-user']`). A cross-session approve / reject /
  submit now propagates to the `/tickets` approvals queue and every
  editor publish-state surface without a manual refresh.
