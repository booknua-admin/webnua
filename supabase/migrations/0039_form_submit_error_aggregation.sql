-- =============================================================================
-- Webnua backend — surface `form_submit_error` in the funnel rollup.
--
-- Pairs with 0038 (the enum value add). Extends `private.aggregate_analytics()`
-- to map `form_submit_error` events to a new `form_failed` stage in
-- `analytics_funnel_daily`. The raw event window (90 days) means the rollup
-- back-fills any errors collected since 0038 was applied.
--
-- Re-running this migration is safe (`create or replace function`). The
-- pg_cron job from 0035 picks up the new function body on its next tick.
-- =============================================================================
