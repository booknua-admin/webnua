-- =============================================================================
-- 0099 — Swap the Webnua brand footer on the two platform_email_templates
-- rows (lead_notification, lead_digest) from the "© Webnua · Perth" text
-- footer to the logo-image footer that matches the rest of the branded
-- operator-email family.
--
-- The logo asset lives at /webnua-logo.png on the deployment (public/).
-- All operator-facing emails use the same `EMAIL_BRAND_FOOTER` HTML block
-- in code; this migration brings the live DB seeded rows in lockstep.
-- =============================================================================
...
update public.platform_email_templates
   set body_html =
        ...
        '</p>' ||
        '</div>' ||
        '<div style="text-align:center;margin-top:18px;padding:0 16px;">' ||
        '<img src="https://app.webnua.com/webnua-logo.png" width="100" alt="Webnua" style="display:inline-block;height:auto;border:0;outline:none;text-decoration:none;opacity:0.7;" />' ||
        '</div>' ||
        '</body></html>',
       body_text =
